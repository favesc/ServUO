...
namespace Server.Gumps
{
    public class GetPetGump : Gump
    {
        ...
        private int m_Pet;
        private int m_cycle;
        public GetPetGump(int pet) : base(250, 60)
        {
            m_Pet = pet;
            m_Price = 5000 * m_Pet;////Price per Pet 3500
            ...
            AddHtml(65, 178, 300, 25, "<BASEFONT COLOR=White>我自己找去.</BASEFONT>", false, false);

            AddHtml(30, 20, 360, 35, "<BASEFONT COLOR=Red>帮你找回宠物，需要付钱！每一只宠物都得付钱！</BASEFONT>", false, false);
            ...
            AddLabel(65, 70, 1645, m_Price.ToString());
            AddLabel(100, 70, 1645, " gold coins for " + m_Pet + " pets found in world");
                ...
                if (m_Price > 0)
                {
                    if (info.IsSwitched(1))
                    {
                        ...
                        foreach (Mobile m in World.Mobiles.Values)
                        {
                            ...
                            if (m is IMount && ((IMount)m).Rider == from)//zp添加的正在骑乘的不算数
                            {
                                pets.Remove(m);
                                //continue;
                            }
                            ...
                            if (m_cycle == 0)
                            {
                                if (Banker.Withdraw(from, m_Price))
                                {
                                    ...
                                    for (int i = 0; i < pets.Count; ++i)
                                    {
                                        ...
                                        if (pet is IMount && ((IMount)pet).Rider == from)
                                        {
                                            pets.Remove(pet);
                                            continue;
                                        }//zp 正在骑乘的宠物不被传送，但 钱一样扣。
                                        //if (pet is IMount)
                                        //    ((IMount)pet).Rider = null; // make sure it's dismounted
