...
namespace Server.Engines.Harvest
{
    public class GraveRobbing : HarvestSystem
    {
        ...
        private GraveRobbing()
        {
            ...
            {
                new HarvestResource( 020.0, 010.0, 150.0, "你娃了一些箭头并放进了背包。",     typeof( Arrow ) ),
                new HarvestResource( 050.0, 030.0, 150.0, "你找到一些弩箭，并放进了背包。",        typeof( Bolt ) ),
                new HarvestResource( 050.0, 030.0, 150.0, "绷带？",        typeof( Bandage ) ,  typeof( VengefulSpirit )),
                new HarvestResource( 050.0, 030.0, 150.0, "你找到了一瓶毒药。",      typeof( DeadlyPoisonPotion ) ,  typeof( VengefulSpirit ) ),
                new HarvestResource( 065.0, 040.0, 150.0, "有一瓶",  typeof( DeadlyPoisonPotion )  ,  typeof( VengefulSpirit ) ),
                new HarvestResource( 065.0, 040.0, 150.0, "又一瓶",  typeof( DeadlyPoisonPotion ) ,    typeof( VengefulSpirit )),
        ...
        {
            from.SendMessage("你不能挖这里。");
        }

        public override void OnHarvestStarted(Mobile from, Item tool, HarvestDefinition def, object toHarvest)
        {
            from.Animate(11, 1, 1, true, false, 0);  //zp
                        ...
                        try
                        {
                            ...
                            if ((stealingskill < 60) ? (0.22 > Utility.RandomDouble()) : (0.17 > Utility.RandomDouble()))
                            {
                                ...
                                if (goodies != null)
                                {
                                    ...
                                    from.SendMessage("有意思了，you dig up something interesting");
                                    from.SendGump(new Gumps.totGump(goodies));//zp
